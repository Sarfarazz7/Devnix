...
// 11. [x] Add selImages to jState and update initialization logic
// 12. [/] Implement image processing (resize & base64) utility function
// ─────────────────────────────────────────────────────────
...
function ensureFinMonthSelector() {
    ...
    if (bar && !document.getElementById('finMonthPick')) {
        ...
        wrap.innerHTML = ` 
            ...
            <button class="fin-today-btn" id="finJumpToday">Today</button>
        `;
        ...
        const jumpBtn = document.getElementById('finJumpToday');
        if (jumpBtn) jumpBtn.onclick = () => {
            const now = new Date().toISOString().slice(0, 7);
            if (S.finMonth === now) return;
            picker.value = now;
...
let jState = { mode: 'list', tab: 'entries', editId: null, selMood: null, selTags: [], selImages: [], searchQ: '' };
...
async function processJournalImages(files) {
    const results = [];
...
function renderJournalPage() {
        ...
        if (!filtered.length) {
            ...
            filtered.forEach(entry => {
                ...
                card.addEventListener('click', e => {
                    ...
                    jState.editId = entry.id; jState.mode = 'view'; jState.selImages = [...(entry.images || [])]; renderJournalPage();
                });
    ...
    sidebar.addEventListener('click', async e => {
        ...
        else if (t.id === 'jNewBtn') { jState.tab = 'entries'; jState.mode = 'edit'; jState.editId = null; jState.selMood = null; jState.selTags = []; jState.selImages = []; renderJournalPage(); }
    });
                    ...
                    options: {
                        ...
                        scales: {
                            y: {
                                min: 0, max: 7,
                                ticks: {
                                    stepSize: 1, font: { size: 10 }, color: tc(),
                                    callback: v => (['', '😢', '😞', '😐', '🙂', '😊', '😄', '🤩'][v] || '')
                                },
                                grid: { color: gc() }
                            },
...
function renderJournalEditorForm(wrap) {
    ...
    if (addImgBtn && imgInp) {
        addImgBtn.onclick = () => imgInp.click();
...
function renderJournalBookView(wrap, entry) {
    ...
    wrap.innerHTML = `
    <div class="journal-book-view">
      <div class="jbv-head">
        <button class="jbv-back" id="jbvBack" title="Back to list">‹</button>
        <div class="jbv-meta">
    ...
    window.openLightbox = (src) => {
        ...
        lb.className = 'jbv-lightbox';
        lb.innerHTML = `<div class="lb-content"><img src="${src}"/><button class="lb-close">×</button></div>`;
