// public/js/app.js
// ── Devnix Frontend — MongoDB-backed version ───────────────
// All data operations go through window.API (api.js).
// S holds the in-memory user snapshot; every mutation hits the
// backend first, then updates S and re-renders.
...
// ── Debounced save of check/notes/skipped maps ─────────────
...
document.querySelectorAll('.nb-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    ['pgDashboard','pgFinance','pgJournal','pgProfile'].forEach(id => document.getElementById(id).classList.add('H'));
...
document.querySelectorAll('.fin-tab').forEach(t => {
  t.addEventListener('click', () => {
    ...
    t.classList.add('on');
    ['ftOverview','ftAnalytics','ftTransactions','ftReview'].forEach(id => document.getElementById(id).classList.add('H'));
...
function renderGrid() {
  ...
  u.tasks.forEach((task, ri) => {
    ...
    dates.forEach(d => {
      ...
      tip.textContent = (isPast?'🔒 Locked — ':(isSkip?'Skipped':isDone?'Done':'Pending')+' — ')+d.toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'short'})+(isDone?' ✓':isSkip?' (skipped)':isPast?' (missed)':'')+(noteVal?' · '+noteVal.slice(0,24):'');
      box.appendChild(tip);
      if (!isPast) {
        box.addEventListener('contextmenu', ev => {
          ev.preventDefault();
          if (isSkip) delete u.skipped[key]; else { u.skipped[key]=true; delete u.done[key]; }
          debounceSave(); renderGrid(); renderDisciplineAnalytics();
        });
...
function renderDisciplineAnalytics() {
  ...
  document.querySelectorAll('.pill[data-d]').forEach(p=>{
    p.addEventListener('click',()=>{document.querySelectorAll('.pill[data-d]').forEach(x=>x.classList.remove('on'));p.classList.add('on');analyDays=+p.dataset.d;renderDisciplineAnalytics();});
  });
  const dk=S.dark,gc=dk?'rgba(255,255,255,.05)':'rgba(0,0,0,.04)',tc=dk?'#555e72':'#9ea3b5';
  document.getElementById('barSkel').classList.add('H');document.getElementById('barWrap').classList.remove('H');
  if(barCI)barCI.destroy();
  barCI=new Chart(document.getElementById('barCh'),{type:'bar',data:{labels:daysArr.map(d=>{const x=new Date(d+'T12:00:00');return WD[x.getDay()]+' '+x.getDate();}),datasets:[{data:pcts,backgroundColor:pcts.map(v=>v===100?'#2e7d32':v>=50?'#66bb6a':'#a5d6a7'),borderRadius:4,borderSkipped:false}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:700,easing:'easeOutQuart'},plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>c.parsed.y+'%'}}},scales:{y:{min:0,max:100,ticks:{callback:v=>v+'%',font:{size:10},color:tc},grid:{color:gc}},x:{ticks:{font:{size:9},color:tc,autoSkip:true,maxTicksLimit:14,maxRotation:45},grid:{display:false}}}}});
  const ma7=pcts.map((_,i)=>{const sl=pcts.slice(Math.max(0,i-6),i+1);return Math.round(sl.reduce((a,b)=>a+b,0)/sl.length);});
  document.getElementById('lineSkel').classList.add('H');document.getElementById('lineWrap').classList.remove('H');
  if(lineCI)lineCI.destroy();
  lineCI=new Chart(document.getElementById('lineCh'),{type:'line',data:{labels:daysArr.map(d=>{const x=new Date(d+'T12:00:00');return MO[x.getMonth()]+' '+x.getDate();}),datasets:[{label:'Daily',data:pcts,borderColor:'#a5d6a7',backgroundColor:'transparent',borderWidth:1.5,pointRadius:2,borderDash:[5,3],tension:.3},{label:'7-day avg',data:ma7,borderColor:'#2e7d32',backgroundColor:'rgba(46,125,50,.08)',borderWidth:2.5,fill:true,pointRadius:3,tension:.4}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:800,easing:'easeOutQuart'},plugins:{legend:{display:false}},scales:{y:{min:0,max:100,ticks:{callback:v=>v+'%',font:{size:10},color:tc},grid:{color:gc}},x:{ticks:{font:{size:9},color:tc,autoSkip:true,maxTicksLimit:10},grid:{display:false}}}}});
  const hm=document.getElementById('hmGrid');hm.innerHTML='';
  for(let i=89;i>=0;i--){const d=dStr(addD(new Date(),-i));const p=u.tasks.length?u.tasks.filter(t=>u.done[t.id+'_'+d]).length/u.tasks.length:0;const sq=document.createElement('div');sq.className='hm-sq hm'+(p===0?0:p<.5?1:p<1?2:3);sq.title=d+': '+Math.round(p*100)+'%';hm.appendChild(sq);}
  document.getElementById('hmSkel').classList.add('H');document.getElementById('hmGrid').classList.remove('H');
...
function destroyFinChart(id){if(finCharts[id]){finCharts[id].destroy();delete finCharts[id];}}
...
function getBudgets(){const u=usr();if(!u)return{};return u.budgets||{};}
...
function renderFinOverview(){
  ...
  if(net<0)insights.push({icon:'<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/>',color:'#ef4444',bg:isDk()?'#2d0f0f':'#fef2f2',title:'Spending exceeds income',body:'You\'re spending '+fmtCurrency(Math.abs(net))+' more than you earn. Review your top categories.'});
...
function refreshAnalyticsCharts(txns){
  ...
  const forecastNW=fLabels.map((_,i)=>Math.round(lastNW+(avg6*(i+1)))),forecastSav=fLabels.map(()=>Math.round(avg6)),allFL=[...monthLabels.slice(-3),...fLabels];
  if(finCharts.forecast){finCharts.forecast.data.labels=allFL;finCharts.forecast.data.datasets[0].data=[...nwData.slice(-3),...Array(6).fill(null)];finCharts.forecast.data.datasets[1].data=[...Array(2).fill(null),nwData[nwData.length-1],...forecastNW];finCharts.forecast.data.datasets[2].data=[...Array(3).fill(null),...forecastSav];finCharts.forecast.update({duration:700,easing:'easeInOutQuart'});}
  else{destroyFinChart('forecast');finCharts.forecast=new Chart(document.getElementById('forecastCh'),{type:'line',data:{labels:allFL,datasets:[{label:'Net worth (actual)',data:[...nwData.slice(-3),...Array(6).fill(null)],borderColor:'#8b5cf6',borderWidth:2,pointRadius:3,tension:.3,backgroundColor:'transparent'},{label:'Net worth (forecast)',data:[...Array(2).fill(null),nwData[nwData.length-1],...forecastNW],borderColor:'#8b5cf6',borderWidth:2,borderDash:[6,4],pointRadius:3,tension:.3,backgroundColor:'rgba(139,92,246,.06)',fill:true},{label:'Monthly savings',data:[...Array(3).fill(null),...forecastSav],borderColor:'#22c55e',borderWidth:1.5,borderDash:[4,3],pointRadius:2,tension:.3,backgroundColor:'transparent',yAxisID:'y1'}]},options:{responsive:true,maintainAspectRatio:false,animation:{duration:800,easing:'easeInOutQuart'},plugins:{legend:{display:false}},scales:{x:{ticks:{font:{size:10},color:tc()},grid:{display:false}},y:{ticks:{callback:v=>(v>=0?'$':'-$')+Math.abs(v/1000).toFixed(1)+'k',font:{size:10},color:tc()},grid:{color:gc()}},y1:{position:'right',ticks:{callback:v=>'$'+v/1000+'k',font:{size:10},color:'#22c55e'},grid:{display:false}}}}});}
...
function renderSavingsGoals(txns){
  ...
  goals.forEach((g,idx)=>{
    ...
    card.querySelector('.del-goal-btn').addEventListener('click',async()=>{
      ...
      try{const goals2=await API.Goals.remove(g.id);S.user.savingsGoals=goals2;renderSavingsGoals(txns);toast('Goal deleted.','warn');}catch(e2){toast(e2.message,'warn');}
    });
...
async function saveBudget(){
  const cat=document.getElementById('bmCat').value,amt=parseFloat(document.getElementById('bmAmt').value);
  if(!cat||isNaN(amt)||amt<0){toast('Enter a valid amount.','warn');return;}
  try{const budgets=await API.Budgets.set(cat,amt);S.user.budgets=budgets;document.getElementById('budgetModal').classList.add('H');refreshAnalyticsCharts(S.user.transactions||[]);toast('Budget set for '+cat+'!');}
  catch(e){toast(e.message,'warn');}
}
...
function exportTxCSV(txList,filename){const header='Date,Description,Category,Type,Amount,Status';const rows=txList.map(t=>`${t.date},"${t.name}",${t.cat},${t.type||(t.amt>0?'income':'expense')},${Math.abs(t.amt).toFixed(2)},${t.status||'cleared'}`);const csv=header+'\n'+rows.join('\n');const a=document.createElement('a');a.href='data:text/csv;charset=utf-8,'+encodeURIComponent(csv);a.download=filename;a.click();}

function renderTxPage(){
  document.getElementById('qaDate').value=today();document.getElementById('qaAdd').onclick=addTransaction;document.getElementById('txSearch').oninput=renderTxTable;document.getElementById('txCatF').onchange=renderTxTable;document.getElementById('txTypeF').onchange=renderTxTable;
  document.getElementById('selEditBtn').onclick=openEditModal;document.getElementById('selDupBtn').onclick=duplicateSelected;document.getElementById('selDelBtn').onclick=deleteSelected;document.getElementById('selClearedBtn').onclick=()=>bulkSetStatus('cleared');document.getElementById('selPendingBtn').onclick=()=>bulkSetStatus('pending');document.getElementById('selExportBtn').onclick=exportSelected;document.getElementById('selClearBtn').onclick=()=>{selectedTxIds.clear();updateSelBar();renderTxTable();};
  document.getElementById('exportAllBtn').onclick=()=>{const u=usr();if(!u)return;const search=(document.getElementById('txSearch')?.value||'').toLowerCase();const cat=document.getElementById('txCatF')?.value||'all';const type=document.getElementById('txTypeF')?.value||'all';const rows=(u.transactions||[]).filter(t=>(cat==='all'||t.cat===cat)&&(type==='all'||t.type===type)&&(t.name.toLowerCase().includes(search)||t.cat.toLowerCase().includes(search)));exportTxCSV(rows,'transactions.csv');toast('Exported '+rows.length+' transactions!');};
  document.getElementById('etCancel').onclick=()=>document.getElementById('editTxModal').classList.add('H');document.getElementById('etSave').onclick=saveEditedTx;
  renderTxTable();
}
...
function openEditModal(){
  const u=usr();if(!u||selectedTxIds.size!==1)return;
  const id=[...selectedTxIds][0];const tx=(u.transactions||[]).find(t=>t.id===id);if(!tx)return;
  document.getElementById('etDesc').value=tx.name;document.getElementById('etAmt').value=Math.abs(tx.amt);document.getElementById('etType').value=tx.type||(tx.amt>0?'income':'expense');document.getElementById('etCat').value=tx.cat;document.getElementById('etDate').value=tx.date;document.getElementById('etStatus').value=tx.status||'cleared';
  document.getElementById('editTxModal').classList.remove('H');document.getElementById('editTxModal').dataset.txid=id;setTimeout(()=>document.getElementById('etDesc').focus(),50);
}

async function saveEditedTx(){
  const u=usr();if(!u)return;
  const id=document.getElementById('editTxModal').dataset.txid;const tx=(u.transactions||[]).find(t=>t.id===id);if(!tx)return;
...
async function addTransaction(){
  const u=usr();if(!u)return;
  const desc=document.getElementById('qaDesc').value.trim();const amt=parseFloat(document.getElementById('qaAmt').value);
  const cat=document.getElementById('qaCat').value;const type=document.getElementById('qaType').value;const date=document.getElementById('qaDate').value||today();
  ...
  try{
    ...
    S.user.transactions=txs;document.getElementById('qaDesc').value='';document.getElementById('qaAmt').value='';
...
function renderTxTable(){
  ...
  const search=(document.getElementById('txSearch')?.value||'').toLowerCase();const cat=document.getElementById('txCatF')?.value||'all';const type=document.getElementById('txTypeF')?.value||'all';
  ...
  rows=[...rows].sort((a,b)=>{let av=a[txSortCol==='description'?'name':txSortCol==='category'?'cat':txSortCol]||'',bv=b[txSortCol==='description'?'name':txSortCol==='category'?'cat':txSortCol]||'';if(txSortCol==='amount'){av=a.amt;bv=b.amt;}if(typeof av==='string')return txSortAsc?av.localeCompare(bv):bv.localeCompare(av);return txSortAsc?av-bv:bv-av;});
  const countEl=document.getElementById('txRowCount');if(countEl)countEl.textContent=rows.length+' of '+txns.length;
  const allIds=rows.map(t=>t.id);const allChecked=allIds.length>0&&allIds.every(id=>selectedTxIds.has(id));
  ...
  const cbAll=document.getElementById('txCbAll');if(cbAll)cbAll.addEventListener('change',()=>{if(cbAll.checked)allIds.forEach(id=>selectedTxIds.add(id));else allIds.forEach(id=>selectedTxIds.delete(id));updateSelBar();renderTxTable();});
  ...
  const selRows=rows.filter(t=>selectedTxIds.has(t.id));const selInc=selRows.filter(t=>t.amt>0).reduce((s,t)=>s+t.amt,0);const selExp=selRows.filter(t=>t.amt<0).reduce((s,t)=>s+Math.abs(t.amt),0);
  body.innerHTML='';
  rows.forEach(t=>{const col=CAT_COLORS[t.cat]||'#888';const bg=isDk()?col+'22':col+'18';const pos=t.amt>0;const sb=t.status==='cleared'?(isDk()?'#0d2818':'#f0fdf4'):(isDk()?'#2e1e00':'#fffbeb');const sc=t.status==='cleared'?(isDk()?'#3fb950':'#15803d'):(isDk()?'#d29922':'#b45309');const isSel=selectedTxIds.has(t.id);const tr=document.createElement('tr');if(isSel)tr.classList.add('tx-selected');tr.style.cursor='pointer';tr.title='Double-click to edit';tr.innerHTML=`<td class="tx-cb-td"><input type="checkbox" class="tx-cb tx-row-cb" data-id="${t.id}" ${isSel?'checked':''}/></td><td style="color:var(--tx3);font-size:11px">${t.date}</td><td><div class="tx-row"><div class="tx-icon" style="background:${bg};color:${col}"><svg viewBox="0 0 24 24" style="width:13px;height:13px;fill:${col}">${CAT_ICONS[t.cat]||''}</svg></div><div class="tx-name">${t.name}</div></div></td><td><span class="badge" style="background:${bg};color:${col}">${t.cat}</span></td><td class="${pos?'amt-pos':'amt-neg'}" style="font-variant-numeric:tabular-nums">${pos?'+':'-'}\$${Math.abs(t.amt).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})}</td><td><span class="badge" style="background:${sb};color:${sc}">${t.status}</span></td>`;tr.addEventListener('dblclick',e=>{if(e.target.closest('.tx-cb-td'))return;selectedTxIds.clear();selectedTxIds.add(t.id);updateSelBar();renderTxTable();openEditModal();});body.appendChild(tr);});
  body.querySelectorAll('.tx-row-cb').forEach(cb=>{cb.addEventListener('change',()=>{const id=cb.dataset.id;if(cb.checked)selectedTxIds.add(id);else selectedTxIds.delete(id);updateSelBar();renderTxTable();});cb.addEventListener('click',e=>e.stopPropagation());});
  if(selRows.length>0){const foot=document.createElement('tr');foot.innerHTML=`<td colspan="6" style="padding:8px 14px;background:var(--blubg);border-top:1px solid var(--blumid);font-size:11px;color:var(--blumid);font-weight:600">Selected: ${selRows.length} row${selRows.length>1?'s':''}${selInc>0?` &nbsp;·&nbsp; <span style="color:var(--grnmid)">+$${selInc.toLocaleString('en-US',{minimumFractionDigits:2})}</span>`:''}${selExp>0?` &nbsp;·&nbsp; <span style="color:var(--redm)">-$${selExp.toLocaleString('en-US',{minimumFractionDigits:2})}</span>`:''} &nbsp;·&nbsp; Net: <span style="color:${selInc-selExp>=0?'var(--grnmid)':'var(--redm)'}">${selInc-selExp>=0?'+':''}\$${Math.abs(selInc-selExp).toLocaleString('en-US',{minimumFractionDigits:2})}</span></td>`;body.appendChild(foot);}
}

function renderWeeklyReview(){
  const u=usr();if(!u)return;const txns=u.transactions||[];const now=new Date();const weekStart=new Date(now);weekStart.setDate(now.getDate()-7);const weekTxns=txns.filter(t=>new Date(t.date)>=weekStart);const weekInc=weekTxns.filter(t=>t.amt>0).reduce((s,t)=>s+t.amt,0);const weekExp=weekTxns.filter(t=>t.amt<0).reduce((s,t)=>s+Math.abs(t.amt),0);const weekNet=weekInc-weekExp;
  ...
  document.getElementById('wrGrid').innerHTML=[{label:'Week income',val:'$'+weekInc.toFixed(0),color:'#22c55e'},{label:'Week expenses',val:'$'+weekExp.toFixed(0),color:'#ef4444'},{label:'Net this week',val:(weekNet>=0?'+':'')+('$'+Math.abs(weekNet).toFixed(0)),color:weekNet>=0?'#22c55e':'#ef4444'},{label:'Transactions',val:weekTxns.length,color:'#3b82f6'}].map(i=>`<div class="wr-item" style="border-color:${i.color}"><div class="wr-item-label">${i.label}</div><div class="wr-item-val" style="color:${i.color}">${i.val}</div></div>`).join('');
  const suggestions=[weekExp>weekInc*0.7?{icon:'⚠️',msg:'Expenses are above 70% of income this week. Consider cutting discretionary spend.',color:'#f59e0b'}:null,weekNet<0?{icon:'🔴',msg:'You spent more than you earned this week. Review non-essential categories.',color:'#ef4444'}:null,weekNet>1000?{icon:'✅',msg:'Great week! You saved $'+weekNet.toFixed(0)+'. Consider moving the surplus to a savings goal.',color:'#22c55e'}:null,{icon:'💡',msg:'Set a daily spending limit to stay on track. Try the 24-hour rule before non-essential purchases.',color:'#3b82f6'},weekTxns.filter(t=>t.cat==='Entertainment').length>3?{icon:'🎬',msg:'Multiple entertainment transactions this week. Check if any subscriptions can be paused.',color:'#8b5cf6'}:null].filter(Boolean);
  ...
  const smallTxns=txns.filter(t=>t.amt<0&&Math.abs(t.amt)<50);const leakMap={};smallTxns.forEach(t=>{leakMap[t.cat]=(leakMap[t.cat]||0)+Math.abs(t.amt);});const leaks=Object.entries(leakMap).sort((a,b)=>b[1]-a[1]).slice(0,4);
  ...
  const catMap2={};txns.filter(t=>t.type==='expense').forEach(t=>{catMap2[t.cat]=(catMap2[t.cat]||0)+Math.abs(t.amt);});const budgets=getBudgets();
  const alerts=[];Object.entries(budgets).forEach(([cat,budget])=>{const spent=catMap2[cat]||0;if(spent>budget)alerts.push({msg:`${cat} is ${Math.round((spent/budget-1)*100)}% over budget`,color:'#ef4444'});});if(weekExp>weekInc)alerts.push({msg:'Overspending detected this week',color:'#ef4444'});alerts.push({msg:'Tip: log transactions daily for more accurate weekly reviews',color:'#3b82f6'});
  ...
  document.getElementById('budgetActual').innerHTML=Object.entries(budgets).map(([cat,budget])=>{const spent=catMap2[cat]||0;const pct=Math.round(spent/budget*100);const over=spent>budget;const color=CAT_COLORS_AN[cat]||'#888';return`<div class="cat-item2"><div class="cat-head2"><div class="cat-name2"><div class="cat-dot" style="background:${color}"></div>${cat}</div><div style="display:flex;gap:8px;align-items:center"><span style="font-size:11px;color:${over?'#ef4444':'var(--tx3)'}">$${spent.toFixed(0)} / $${budget}</span><span class="cat-amt" style="color:${over?'#ef4444':color}">${pct}%</span></div></div><div class="cat-track"><div class="cat-fill" style="width:${Math.min(pct,100)}%;background:${over?'#ef4444':color}"></div></div></div>`;}).join('')||'<div style="font-size:12px;color:var(--tx3);padding:8px 0">Set budgets in the Analytics tab to track actuals here.</div>';
}

// ══════════════════════════════════════════════════════════
//  JOURNAL MODULE
// ══════════════════════════════════════════════════════════
const MOODS=[{e:'😊',l:'Happy',score:5},{e:'😌',l:'Calm',score:4},{e:'🔥',l:'Motivated',score:5},{e:'😔',l:'Sad',score:1},{e:'😤',l:'Frustrated',score:2},{e:'😴',l:'Tired',score:2},{e:'🤔',l:'Reflective',score:3},{e:'🎉',l:'Excited',score:5}];
const TAGS=['Gratitude','Work','Personal','Goals','Health','Travel','Learning','Idea'];
const MOOD_COLORS={'😊':'#4caf50','😌':'#2196f3','🔥':'#ff5722','😔':'#9e9e9e','😤':'#f44336','😴':'#7986cb','🤔':'#ff9800','🎉':'#e91e63'};
...
function renderJournalEditorForm(wrap){
  const isEdit=jState.mode==='edit'&&!!jState.editId;const journals=getJournals();const existing=isEdit?journals.find(j=>j.id===jState.editId):null;
  ...
  const bodyInp=wrap.querySelector('#jBodyInp');const wc=wrap.querySelector('#jWC');const updateWC=()=>{const words=(bodyInp.value.trim().match(/\S+/g)||[]).length;wc.textContent=words+' word'+(words!==1?'s':'');};bodyInp.addEventListener('input',updateWC);updateWC();
  ...
  wrap.querySelector('#jDiscard').addEventListener('click',()=>{if(isEdit){jState.mode='view';renderJournalPage();}else{jState.mode='list';jState.editId=null;renderJournalPage();}});
  wrap.querySelector('#jSave').addEventListener('click',async()=>{
    const title=document.getElementById('jTitleInp').value.trim();const body=document.getElementById('jBodyInp').value.trim();
    if(!title&&!body){toast('Write something first!','warn');return;}
    try{
      if(isEdit&&existing){const journals2=await API.Journals.update(existing.id,{title:title||'Untitled',body,mood:jState.selMood,tags:jState.selTags});S.user.journals=journals2;toast('Entry updated! 📝');}
      else{const journals2=await API.Journals.add({title:title||'Untitled',body,mood:jState.selMood,tags:jState.selTags,date:today()});S.user.journals=journals2;jState.editId=journals2[0]?.id||null;toast('Journal saved! 📖');}
      jState.mode='view';jState.selMood=null;jState.selTags=[];renderJournalPage();
    }catch(e){toast(e.message,'warn');}
  });
}

function renderJournalBookView(wrap,entry){
  const wordCount=(entry.body||'').trim().split(/\s+/).filter(Boolean).length;const readMins=Math.max(1,Math.ceil(wordCount/200));
